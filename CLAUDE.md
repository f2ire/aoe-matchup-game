...
`firstHitBlocked` flag (`CombatEntity`): injected in Sandbox.tsx when `ability-deflective-armor` is active on the defender. In `computeMetrics`, takes an early-return path before the normal charge/TTK logic: first hit = 0 damage (charge also nullified), attacker spends `firstHitSpeed`, then all subsequent hits are normal (no charge). `HTK = 1 + ceil(HP / effectiveNormalCycle)`, `TTK = firstHitSpeed + normalHTK × attackSpeed`. Formula prefixed with `Deflect`. Secondary weapons on hits 2+ still apply normally.
...
| `ability-kabura-ya` | Onna-Musha (`ja`, age 3+) | Raw variation effect `change:0` (no-op). Patched to `moveSpeed ×1.1` on `onna-musha`. `active:'manual'` set on both ability and variation — required to bypass `unlockedBy` suppression in `getAbilitiesForUnit` (line 249: abilities with `unlockedBy` pointing to an applicable tech are hidden unless `active === 'manual'`). Duration 10s not modelled. |
| `ability-katana-bannerman-aura` | Japanese (`ja`) | Raw: `meleeAttack ×1.15` on `melee infantry`. `active:'always'` restricted to `katana-bannerman` via `activeForIds`. |
| `ability-yumi-bannerman-aura` | Japanese (`ja`) | Raw: `rangedAttack ×1.15` on `ranged infantry`. `active:'always'` restricted to `yumi-bannerman` via `activeForIds`. |
| `ability-uma-bannerman-aura` | Japanese (`ja`) | Raw: `meleeAttack ×1.10` on `cavalry`. Patched: `rangedAttack ×1.10` added (same select) for mounted ranged units. `active:'always'` restricted to `uma-bannerman` via `activeForIds`. |
| `buddhist-conversion` | Japanese (`ja`, age 3+) | Synthetic. `active:'manual'`. `meleeAttack`, `rangedAttack`, `siegeAttack` ×1.2 targeting `land_military` class. Duration 20s not modelled. Icon: `/abilities/buddhist-conversion.png`. |
| `ability-nehan` | Japanese (`ja`, age 4) | Synthetic. `active:'manual'`. `moveSpeed ×1.25` targeting `land_military` class. Icon: aoe4world tech icon URL. |
| `ability-five-mountain-ministries` | Japanese (`ja`, age 3+) | Raw effects empty. Patched: `versusOpponentDamageDebuff ×0.5` targeting `annihilation_condition` attackers — all enemies deal −50% damage (base + bonus). Duration 60s not modelled. `active:'manual'`. |

**`versusOpponentDamageDebuff` visibility rule:** `abilityAffectsUnit` and `getAbilitiesForUnit` (`unified-abilities.ts`) handle this property specially. `select.id` = specific unit IDs that own the ability; `select.class` = unit classes that can own the ability (visibility). Both are checked — the ability is shown to any unit matching either. In combat (`getVersusDebuffMultiplier`), `select.class` still identifies which *attackers* are debuffed as before. So `select.class: [['annihilation_condition']]` both makes the ability visible for all combat units AND debuffs all combat unit attackers.
...
| `upgrade-shinobi-3/4` | Shinobi (`ja`) | "Shinobi Scaling Technology" auto-upgrades — baked into age-3 and age-4 unit variations instead. Age 3: HP 92, Wakizashi 23, Torch 12 (base×1.15). Age 4: HP 106, Wakizashi 26, Torch 13 (base×1.15²). Techs excluded via `techUnitExclusions` (`excludedUnits:['shinobi']`). |
...
`Ability.activeForIds?: string[]` — when set on an ability with `active:'always'`, auto-activation is restricted to units whose `id` or `baseId` is in the list. Units not in the list see the ability as manual. Set via `after` in the ability patch. Both auto-activate filters in `useUnitSlot.ts` (weapon-swap path and default path) check this field.
- To restrict auto-activation to specific units: set `activeForIds: ['unit-id']` in the ability patch's `after` function.
- Current use: `ability-katana-bannerman-aura` → `activeForIds: ['katana-bannerman']`.
...
- `charge-attack` is explicitly excluded from desert-raider, cataphract, camel-rider, black-rider, and shinobi `abilities` (none can charge despite having `melee`/`knight` class). Filter is in `useUnitSlot.ts` abilities memo.
...
- **Note:** `moveSpeed` previously had a special case where `"change"` was treated as a percentage (`stat *= 1 + value/100`). This is now commented out. `do-maru-armor` (patched to `multiply: 1.1`) and `kabura-ya-whistling-arrow` (effects cleared, covered by ability) were the two affected raw techs using `change: 10`. Any future raw tech with `moveSpeed change: N` for a percentage must be patched to `multiply: 1 + N/100`.
